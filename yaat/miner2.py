from yaat.util import fetchjson
...
class Miner:
    ...
    @classmethod
    def mine_alpha(cls, interv_min:int=15, start:datetime=datetime(2023, 2, 2, tzinfo=Maester.tz), end:datetime=datetime(2023, 4, 2, tzinfo=Maester.tz),
                   syms:List[str]=list({'SPY', 'XLK', 'XLV', 'XLY', 'IBB', 'XLF', 'XLP', 'XLE', 'XLU', 'XLI','XLB'})):
        assert interv_min in (1, 5, 15, 30, 60), "valid minute intervals are 1, 5, 15, 30, 60"

        # parameter cleaning
        syms = list(set(syms))
        start = start.astimezone(Maester.tz)
        end = end.astimezone(Maester.tz)
        ...
        existing_ticks = list(Maester.tickers_coll.aggregate([
            {'$match': {
                ...
                '$expr': {'$and': [
                    ...
                    {'$in': [{'$minute': '$datetime'}, list(range(0, 60, interv_min))]}
                ]}
            }},
        ]))

        # get misssing symbols and datetimes
        missing_ticks: List[Dict] = []
        desired_ints = set(pd.date_range(start=start, end=end, freq=f"{interv_min}min"))
        ...
        for _, mt in missing_ticks.iterrows():
            res = cls.call_alpha(function='TIME_SERIES_INTRADAY', interval=f'{interv_min}min', symbol=mt['symbol'], month=f"{mt['year']}-{mt['month']:02}")
            ...
            for time, ohlcv in tickers.items():
                ...
                if dt not in mt['datetime']:
                    ...
                    except DuplicateKeyError: pass # something is not right with pandas agg() but it is fine
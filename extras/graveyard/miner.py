...
class Miner:
    ...
    def mine(self, stock_symbols:List[str]=['SPY', 'XLK', 'XLV', 'XLY', 'IBB', 'XLF', 'XLP', 'XLE', 'XLU', 'XLI','XLB'], # 'XLRE'
             currency_symbols:List[str]=['BTC-USD', 'ETH-USD', 'DX-Y.NYB', 'EURUSD=X', 'USDCNY=X', 'GC=F', 'CL=F'],
             stop_year:int = 2024, stop_month:int=1):
        assert 0 < stop_month <= 12 and 2024 <= stop_year
